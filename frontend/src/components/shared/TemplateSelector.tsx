...
const presetTemplates = [
  { id: '1', name: '復古捲軸', preview: '/templates/template_y.png', thumb: '/templates/template_y-thumb.webp' },
  { id: '2', name: '矢量插畫', preview: '/templates/template_vector_illustration.png', thumb: '/templates/template_vector_illustration-thumb.webp' },
...
interface TemplateSelectorProps {
  ...
  showUpload?: boolean; // 是否顯示上傳到用戶模板庫的選項
  projectId?: string | null; // 項目ID，用於素材選擇器
}
...
export const TemplateSelector: React.FC<TemplateSelectorProps> = ({
  ...
  const [saveToLibrary, setSaveToLibrary] = useState(true); // 上傳模板時是否保存到模板庫（默認勾選）
    ...
    if (file) {
      try {
        if (showUpload) {
          ...
          // 預覽頁模式：根據 saveToLibrary 狀態決定是否保存到模板庫
          if (saveToLibrary) {
            // 保存到模板庫並應用
            const response = await uploadUserTemplate(file);
            if (response.data) {
              ...
              show({ message: '模板已保存到模板庫', type: 'success' });
            }
          } else {
            // 僅應用到項目
            onSelect(file);
  ...
  const handleSelectMaterials = async (materials: Material[], saveAsTemplate?: boolean) => {
    ...
    try {
      ...
      // 根據 saveAsTemplate 參數決定是否保存到模板庫
      if (saveAsTemplate) {
        // 保存到用戶模板庫
        const response = await uploadUserTemplate(file);
        if (response.data) {
          ...
          // 傳遞文件和模板ID，適配不同的使用場景
          onSelect(file, template.template_id);
          show({ message: '素材已保存到模板庫', type: 'success' });
        }
      } else {
        // 僅作為模板使用
        onSelect(file);
      ...
      show({ message: '加載素材失敗: ' + (error.message || '未知錯誤'), type: 'error' });
    }
  };
...
/**
 * 根據模板ID獲取模板File對象（按需加載）
 * @param templateId 模板ID
 * @param userTemplates 用戶模板列表
