...
export const MaterialSelector: React.FC<MaterialSelectorProps> = ({
  ...
  const [filterProjectId, setFilterProjectId] = useState<string>('all'); // 始終默認顯示所有素材
  const [projects, setProjects] = useState<Project[]>([]);
  const [projectsLoaded, setProjectsLoaded] = useState(false);
  const [isGeneratorOpen, setIsGeneratorOpen] = useState(false);
  const [saveAsTemplate, setSaveAsTemplate] = useState(true); // 默認勾選：保存為模板
  const [showAllProjects, setShowAllProjects] = useState(false); // 控制是否顯示所有項目

  // 不再根據 projectId 自動改變篩選，用戶可以自己選擇
  ...
  const loadMaterials = async () => {
    setIsLoading(true);
    try {
      // 如果 filterProjectId 是 'all'，傳遞 'all'；如果是 'none'，傳遞 'none'；否則傳遞實際的項目ID
      ...
      console.error('加載素材列表失敗:', error);
      show({
        message: error?.response?.data?.error?.message || error.message || '加載素材列表失敗',
        type: 'error',
      });
    } finally {
      setIsLoading(false);
    ...
    if (multiple) {
      ...
      if (newSelected.has(key)) {
        ...
        if (maxSelection && newSelected.size >= maxSelection) {
          show({
            message: `最多只能選擇 ${maxSelection} 個素材`,
            type: 'info',
          });
          return;
        }
  ...
  const handleConfirm = () => {
    ...
    // 如果啓用了保存為模板選項，傳遞saveAsTemplate狀態
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!allowedTypes.includes(file.type)) {
      show({ message: '不支持的圖片格式', type: 'error' });
      return;
    }

    setIsUploading(true);
    try {
      // 簡化上傳邏輯：在 'all' 或 'none' 篩選時上傳為全局素材（不關聯項目）
          ...
          {isLoading && materials.length === 0 ? (
            ...
            <div className="flex flex-col items-center justify-center py-12 text-gray-400 p-4">
              ...
              <div className="text-xs mt-1">
                {projectId ? '可以上傳圖片或使用素材生成功能創建素材' : '可以上傳圖片作為素材'}
              </div>
            </div>
          ) : (
