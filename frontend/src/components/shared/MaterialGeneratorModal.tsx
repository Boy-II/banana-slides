...
interface MaterialGeneratorModalProps {
  projectId?: string | null; // 可選，如果不提供則生成全局素材
  isOpen: boolean;
  onClose: () => void;
}
 ...
 * - 提示詞原樣傳給文生圖模型（不做額外修飾）
 * - 生成結果展示在模態頂部
 * - 結果統一保存在項目下的歷史素材庫（backend /uploads/{projectId}/materials）
 */
export const MaterialGeneratorModal: React.FC<MaterialGeneratorModalProps> = ({
  ...
  const handleSelectMaterials = async (materials: Material[]) => {
    try {
      ...
      show({ message: `已添加 ${files.length} 個素材`, type: 'success' });
    } catch (error: any) {
      ...
      try {
        ...
        if (task.status === 'COMPLETED') {
          ...
          if (attempts >= maxAttempts) {
            show({ message: '素材生成超時，請稍後查看素材庫', type: 'warning' });
            setIsGenerating(false);
            if (pollingIntervalRef.current) {
              clearInterval(pollingIntervalRef.current);
              pollingIntervalRef.current = null;
        ...
        if (attempts >= maxAttempts) {
          show({ message: '輪詢任務狀態失敗，請稍後查看素材庫', type: 'error' });
          setIsGenerating(false);
          if (pollingIntervalRef.current) {
            clearInterval(pollingIntervalRef.current);
            pollingIntervalRef.current = null;
  ...
  const handleGenerate = async () => {
    ...
    try {
      // 如果沒有projectId，使用'none'表示生成全局素材（後端會轉換為'global'用於Task）
