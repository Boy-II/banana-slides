...
interface ReferenceFileListProps {
  // 兩種模式：1. 從 API 加載（傳入 projectId） 2. 直接顯示（傳入 files）
  projectId?: string | null;
  files?: ReferenceFile[]; // 如果傳入 files，則直接顯示，不從 API 加載
...
export const ReferenceFileList: React.FC<ReferenceFileListProps> = ({
  ...
  // 如果傳入了 files，使用外部文件列表；否則從 API 加載
  ...
  useEffect(() => {
    ...
    const loadFiles = async () => {
      try {
        ...
        showRef.current({
          message: error?.response?.data?.error?.message || error.message || '加載文件列表失敗',
          type: 'error',
        });
      }
    };
