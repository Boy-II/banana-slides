...
export const Home: React.FC = () => {
  ...
  // 上傳圖片
  // 在首頁，圖片始終上傳為全域素材（不關聯專案），因為此時還沒有專案
  const handleImageUpload = async (file: File) => {
    ...
    try {
      ...
      if (response?.data?.url) {
        ...
        show({ message: '圖片上傳失敗：未回傳圖片資訊', type: 'error' });
      }
    } catch (error: any) {
  ...
  // 上傳檔案
  // 在首頁，檔案始終上傳為全域檔案（不關聯專案），因為此時還沒有專案
  const handleFileUpload = async (file: File) => {
    ...
    try {
      ...
      if (response?.data?.file) {
        ...
        if (uploadedFile.parse_status === 'pending') {
          try {
            ...
            if (parseResponse?.data?.file) {
              ...
              // 如果沒有回傳檔案物件，手動更新狀態為 parsing（非同步執行緒會稍後更新）
              setReferenceFiles(prev =>
        ...
        show({ message: '檔案上傳失敗：未回傳檔案資訊', type: 'error' });
      }
    } catch (error: any) {
  ...
  // 取得目前已選擇的檔案ID列表，傳遞給選擇器（使用 useMemo 避免每次渲染都重新計算）
  ...
  return (
    <div className="min-h-screen bg-[#e2dccf] relative overflow-hidden">
      {/* 背景装饰元素 */}
