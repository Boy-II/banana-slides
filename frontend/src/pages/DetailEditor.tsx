...
export const DetailEditor: React.FC = () => {
  ...
  useEffect(() => {
    if (projectId && (!currentProject || currentProject.id !== projectId)) {
      ...
      // 如果項目已存在，也同步一次以確保數據是最新的（特別是從描述生成後）
      // 但只在首次加載時同步，避免頻繁請求
  ...
  }, [projectId, currentProject?.id]); // 只在 projectId 或項目ID變化時更新
  ...
  const handleExportDescriptions = useCallback(() => {
    ...
    show({ message: '導出成功', type: 'success' });
  }, [currentProject, show]);
        ...
        <div className="flex items-center justify-between gap-2 md:gap-4">
          ...
          <div className="flex items-center gap-2 md:gap-4 flex-shrink-0">
            ...
            <div className="flex items-center gap-1.5 md:gap-2">
              ...
              <span className="text-base md:text-xl font-bold">PPT生成系統</span>
            </div>
