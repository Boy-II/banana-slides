...
test.describe('UI-driven E2E test: From user interface to PPT export', () => {
  ...
  test('User Full Flow: Create and export PPT in browser', async ({ page }) => {
    ...
    await ideaInput.fill('創建一份關於人工智能基礎的簡短PPT，包含3頁：什麼是AI、AI的應用、AI的未來')
    ...
    // Wait for image generation page to load (button text includes page count like "批量生成圖片 (3)")
    const generateImageBtn = page.locator('button').filter({ hasText: '批量生成圖片' })
    ...
    if (await generateImageBtn.count() > 0) {
      ...
      // Get expected page count from the button text (e.g., "批量生成圖片 (3)")
      ...
      await expect(async () => {
        ...
        const exportBtnCheck = page.locator('button:has-text("導出")')
    ...
    const exportBtn = page.locator('button:has-text("導出")')
    ...
    // Step 2: Click "導出為 PPTX" in the dropdown menu
    const exportPptxBtn = page.locator('button:has-text("導出為 PPTX")')
    await exportPptxBtn.waitFor({ state: 'visible', timeout: 5000 })
    await exportPptxBtn.click()
    console.log('✓ Clicked "導出為 PPTX" button\n')
...
test.describe('UI E2E - Simplified (skip long waits)', () => {
  ...
  test('User flow verification: Only verify UI interactions, do not wait for AI generation', async ({ page }) => {
    ...
    try {
      ...
      try {
        await page.waitForSelector(
          '.animate-spin, button[disabled], div:has-text("加載"), div:has-text("生成中")',
          { timeout: 5000 }
        )
