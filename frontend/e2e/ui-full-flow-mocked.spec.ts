...
test.describe('UI-driven E2E test (Mocked Backend)', () => {
  ...
  test('User Full Flow: Create and export PPT with mocked API', async ({ page }) => {
    ...
    await page.waitForSelector('button:has-text("批量生成圖片")', { timeout: 10000 })
    const generateImageBtn = page.locator('button:has-text("批量生成圖片")')
    ...
    const exportBtn = page.locator('button:has-text("導出"), button:has-text("下載"), button:has-text("完成")')
